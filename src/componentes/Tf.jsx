    ...
    const cargarImagen = () => {
        ...
        imagen.onload = async () => {
            try {
                ...
                const imagenTensor = imagenRedimensionada.expandDims();
                const imagenNormalizada = imagenTensor.div(tf.scalar(255));

                const model = await tf.loadGraphModel('modeljs/model.json');

                let predict12 = model.predict(imagenNormalizada)
                const predictionsArray = await predict12.data();
